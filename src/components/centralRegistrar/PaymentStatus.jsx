...
const ClearanceList = ({ studentsData }) => {
  ...
  const columns = [
    ...
    { title: 'Payment Status', dataIndex: 'paymentStatus', key: 'paymentStatus' },
    {
      ...
      key: 'action',
      render: (text, record) => (
        <Button onClick={() => showClearanceModal(record)}>Issue Clearance</Button>
      ),
    },
  ];
  ...
  const getFilteredStudentRecords = (campus, year) => {
    ...
    console.log("test " ,studentsData.filter(student => student.year))
    if (campus , year) {
        return studentsData.filter((student) => student.department == campus && student.year == year);
    }else if (campus) {
  ...
  <div className="list-filter">
    ...
    <Select
      ...
      <Table columns={columns} dataSource={getFilteredStudentRecords(selectedCampus, selectedYear)} pagination={{ position: ['bottomCenter'] }} />
...
const studentsData = [
  { id: 1, name: 'John Doe', year :2012 , department:'computerscience',paymentStatus: 'Paid' },
  { id: 2, name: 'Job Smith', year :2011 , department:'informationscience', paymentStatus: 'Unpaid' },
  { id: 3, name: 'Aman Weye', year :2011 , department:'Electrical', paymentStatus: 'Unpaid' },
  { id: 4, name: 'Sole Exo', year :2011 , department:'Accounting', paymentStatus: 'Paid' },
];
