...
const GradeSubmission = () => {
  ...
  useEffect(() => {
    const fetchStudentMark = async () => {
      try {
        const excludedResponse = await axiosInstance.get(`/api/StudentMarks`); // Replace with your course API endpoint
  ...
  const transposeData = (courseNo, term, empId) => {
    ...
    for (let i = 1; i < dataSource.length; i++) {
      ...
      for (let j = 2; j < row.length - 1; j++) {
        // Skipping No., StudID, FullName, and total
  ...
  const handleSubmitData = async () => {
    ...
    if (filteredData.length > 0) {
      ...
      console.log(
        "sort is is ",
        transposeData(firstItem.courseNo, firstItem.termId, firstItem.empId)
      );
      for (
        let i = 0;
        i <
      ...
      console.log(
        "sort is is ",
        transposeDatas(firstItem.courseNo, firstItem.termId, firstItem.empId)
      );
    } else {
  ...
  const filteredAssessmentData = assesment.filter(
    (item) => item.courseNo === course
  );

  // Extract assessment titles from filtered data
  const assessmentTitles = filteredAssessmentData.map(
        ...
        <div style={{ display: "none" }} ref={tableRef}>
          <table>
            <thead>
              ...
              <tr>
                <th colSpan="2">
                  Term: {semester}, Section: {academicYear}, Instructor: {empId}
                </th>
              </tr>
              <tr>
                <th className="bg-[#4d6a90]">No.</th>
                <th>StudID</th>
              </tr>
            </thead>
          </table>
